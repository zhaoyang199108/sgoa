...
<html>
	<head>
		...
		$(document).ready(function(){
		    ...
		    $("#addBtn").click(function(){
		        var url = "${appDomain}/admin/sortdetail/add_sortdetail.htm?sortId=" + $(this).attr("alt");
		    ...
		    $(".act-update").click(function(){
		        var url = "${appDomain}/admin/sortdetail/edit_sortdetail.htm?id=" + $(this).attr("alt") + "&r=" + Math.random();
		        var param = 'dialogWidth=720px;dialogHeight=220px;status=no;center=yes;scroll=no';
		        var val = window.showModalDialog(url, '', param);
		        if (val == 'refresh') {
		            alert("操作成功");
		    ...
		    $(".act-delete").click(function(){
		        var url = "${appDomain}/admin/sortdetail/delete_sortdetail.htm?id=" + $(this).attr("alt");
		        if (confirm("确定删除该分类？")) {
		    ...
		    $("#delBatchBtn").click(function(){
		        ...
		        if (confirm("确定删除所选分类?")) {
		            ...
		            var url = "${appDomain}/admin/sortdetail/delete_sortdetailArray.htm"
		            $.get(url, {
		                idArray: value
		            }, function(result){
		                alert("操作成功");
		...
		var url = "${appDomain}/admin/sortdetail/sortdetail_detail.htm?id=" + messageId;
    ...
    <body>
        <div id="main">
            <div id="content">    
             <h3 class="title">子类列表页面</h3>              
                <form id="sortdetailForm" name="sortdetailForm" action="${appDomain}/admin/sortdetail/list.htm" method="GET">
	              ...
	              <div class="search">
	                ...
	                <div class="list">                    
                     <div class="opt-btn">
	                        ...
	                        <ul class="action-btn">
	                            <li>
	                                <a id="addBtn" class="action-opt action-new" href="#" alt="${sortId}">新增</a>
	                            </li>
	                            <li>
	                    ...
	                    <table class="list-info">
	                        ...
	                        <#list page.sortdetailList as sortdetail>
								<tr>
									<td width="3%">
		                                <input type="checkbox" name="chkItem" value=${sortdetail.id} />
		                            </td>
									<td>${sortdetail.name}</td>
		                            <td class="td-btn" width="20%">		                           		
										<a class="action-opt act-update" href="#" title="更新" alt="${sortdetail.id}">修改</a>
										<a class="action-opt act-delete" href="#" title="删除" alt="${sortdetail.id}">删除</a>
									</td>
								</tr>
							</#list>
	                        </tr>
