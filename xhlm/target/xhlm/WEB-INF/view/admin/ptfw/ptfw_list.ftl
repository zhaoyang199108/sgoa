...
<html>
	<head>
		...
		$(document).ready(function(){
		    ...
		    $("#addBtn").click(function(){
		        var url = "${appDomain}/admin/ptfw/add_ptfw.htm";
		    ...
		    $(".act-update").click(function(){
		        var url = "${appDomain}/admin/ptfw/edit_ptfw.htm?id=" + $(this).attr("alt") + "&r=" + Math.random();
		        var param = 'dialogWidth=720px;dialogHeight=770px;status=no;center=yes;scroll=no';
		        var val = window.showModalDialog(url, '', param);
		        if (val == 'refresh') {
		            alert("操作成功");
		    ...
		    $(".act-delete").click(function(){
		        var url = "${appDomain}/admin/ptfw/delete_ptfw.htm?id=" + $(this).attr("alt");
		        if (confirm("确定删除该分类？")) {
		    ...
		    $("#delBatchBtn").click(function(){
		        ...
		        if (confirm("确定删除所选分类?")) {
		            ...
		            var url = "${appDomain}/admin/ptfw/delete_ptfwArray.htm"
		            $.get(url, {
		                idArray: value
		            }, function(result){
		                alert("操作成功");
		...
		var url = "${appDomain}/admin/ptfw/ptfw_detail.htm?id=" + messageId;
    ...
    <body>
        <div id="main">
            <div id="content">    
             <h3 class="title">平台服务列表页面</h3>              
                <form id="ptfwForm" name="ptfwForm" action="${appDomain}/admin/ptfw/list.htm" method="GET">                
	              <div class="search">
					<ul>
	                ...
	                <div class="list">                    
                     <div class="opt-btn">
	                    ...
	                    <table class="list-info">
	                        ...
	                        <#list page.ptfwList as ptfw>
								<tr>
									<td width="3%">
		                                <input type="checkbox" name="chkItem" value=${ptfw.id} />
		                            </td>
									<td>${ptfw.title}</td>
									<td>${ptfw.sortName}</td>
									<td>${ptfw.userName}</td>
									<td>${ptfw.createDate?datetime}</td>
		                            <td class="td-btn" width="20%">		                           		
										<a class="action-opt act-update" href="#" title="更新" alt="${ptfw.id}">修改</a>
										<a class="action-opt act-delete" href="#" title="删除" alt="${ptfw.id}">删除</a>
									</td>
								</tr>
							</#list>
	                        </tr>
