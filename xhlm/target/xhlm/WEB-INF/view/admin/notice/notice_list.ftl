...
<html>
	<head>
		...
		$(document).ready(function(){
		    ...
		    $("#addBtn").click(function(){
		        var url = "${appDomain}/admin/notice/add_notice.htm";
		        var param = 'dialogWidth=720px;dialogHeight=570px;status=no;center=yes;scroll=no';
		    ...
		    $(".act-update").click(function(){
		        var url = "${appDomain}/admin/notice/edit_notice.htm?id=" + $(this).attr("alt") + "&r=" + Math.random();
		        var param = 'dialogWidth=720px;dialogHeight=570px;status=no;center=yes;scroll=no';
		        var val = window.showModalDialog(url, '', param);
		        if (val == 'refresh') {
		            alert("操作成功");
		    ...
		    $(".act-role").click(function(){
		        var url = "${appDomain}/admin/notice/notice_role.htm?id=" + $(this).attr("alt") + "&r=" + Math.random();
		    ...
		    $(".act-delete").click(function(){
		        var url = "${appDomain}/admin/notice/delete_notice.htm?id=" + $(this).attr("alt");
		        if (confirm("确定删除该分类？")) {
		    ...
		    $("#delBatchBtn").click(function(){
		        ...
		        if (confirm("确定删除所选分类?")) {
		            ...
		            var url = "${appDomain}/admin/notice/delete_noticeArray.htm"
		            $.get(url, {
		                idArray: value
		            }, function(result){
		                alert("操作成功");
		    ...
		    $("#imBatchBtn").click(function(){
		        ...
		        if (confirm("确定将所选分类注册到环信?")) {
		            ...
		            var url = "${appDomain}/admin/notice/im_notice.htm"
		            $.get(url, {
		                idArray: value
		            }, function(result){
		                alert("操作成功");
		...
		var url = "${appDomain}/admin/notice/notice_detail.htm?id=" + messageId;
    ...
    <body>
        <div id="main">
            <div id="content">    
             <h3 class="title">通知列表页面</h3>              
                <form id="noticeForm" name="noticeForm" action="${appDomain}/admin/notice/list.htm" method="GET">                
	              <div class="search">
					<ul>
                     ...
                     <div class="opt-btn">
	                    ...
	                    <table class="list-info">
	                        ...
	                        <#list page.noticeList as notice>
								<tr>
									<td width="3%">
		                                <input type="checkbox" name="chkItem" value=${notice.id} />
		                            </td>
									<td>${notice.title}</td>
									<td>${notice.sortName}</td>
									<td>${notice.userName}</td>
									<td>${notice.createDate?datetime}</td>
		                            <td class="td-btn" width="20%">		                           		
										<a class="action-opt act-update" href="#" title="更新" alt="${notice.id}">修改</a>
										<a class="action-opt act-delete" href="#" title="删除" alt="${notice.id}">删除</a>
									</td>
								</tr>
							</#list>
	                        </tr>
