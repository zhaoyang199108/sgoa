...
<html>
	<head>
		...
		$(document).ready(function(){
		    ...
		    $("#addBtn").click(function(){
		        var url = "${appDomain}/admin/pthd/add_pthd.htm";
		    ...
		    $(".act-update").click(function(){
		        var url = "${appDomain}/admin/pthd/edit_pthd.htm?id=" + $(this).attr("alt") + "&r=" + Math.random();
		        var param = 'dialogWidth=720px;dialogHeight=770px;status=no;center=yes;scroll=no';
		        var val = window.showModalDialog(url, '', param);
		        if (val == 'refresh') {
		            alert("操作成功");
		    ...
		    $(".act-delete").click(function(){
		        var url = "${appDomain}/admin/pthd/delete_pthd.htm?id=" + $(this).attr("alt");
		        if (confirm("确定删除该分类？")) {
		    ...
		    $("#delBatchBtn").click(function(){
		        ...
		        if (confirm("确定删除所选分类?")) {
		            ...
		            var url = "${appDomain}/admin/pthd/delete_pthdArray.htm"
		            $.get(url, {
		                idArray: value
		            }, function(result){
		                alert("操作成功");
		...
		var url = "${appDomain}/admin/pthd/pthd_detail.htm?id=" + messageId;
    ...
    <body>
        <div id="main">
            <div id="content">    
             <h3 class="title">平台活动列表页面</h3>              
                <form id="pthdForm" name="pthdForm" action="${appDomain}/admin/pthd/list.htm" method="GET">                
	              <div class="search">
					<ul>
	                ...
	                <div class="list">                    
                     <div class="opt-btn">
	                    ...
	                    <table class="list-info">
	                        ...
	                        <#list page.pthdList as pthd>
								<tr>
									<td width="3%">
		                                <input type="checkbox" name="chkItem" value=${pthd.id} />
		                            </td>
									<td>${pthd.title}</td>
									<td>${pthd.sortName}</td>
									<td>${pthd.userName}</td>
									<td>${pthd.createDate?datetime}</td>
		                            <td class="td-btn" width="20%">		                           		
										<a class="action-opt act-update" href="#" title="更新" alt="${pthd.id}">修改</a>
										<a class="action-opt act-delete" href="#" title="删除" alt="${pthd.id}">删除</a>
									</td>
								</tr>
							</#list>
	                        </tr>
