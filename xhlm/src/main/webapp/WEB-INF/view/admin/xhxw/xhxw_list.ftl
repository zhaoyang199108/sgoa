...
<html>
	<head>
		...
		$(document).ready(function(){
		    ...
		    $("#addBtn").click(function(){
		        var url = "${appDomain}/admin/xhxw/add_xhxw.htm";
		    ...
		    $(".act-update").click(function(){
		        var url = "${appDomain}/admin/xhxw/edit_xhxw.htm?id=" + $(this).attr("alt") + "&r=" + Math.random();
		        var param = 'dialogWidth=720px;dialogHeight=600px;status=no;center=yes;scroll=no';
		        var val = window.showModalDialog(url, '', param);
		        if (val == 'refresh') {
		            alert("操作成功");
		    ...
		    $(".act-delete").click(function(){
		        var url = "${appDomain}/admin/xhxw/delete_xhxw.htm?id=" + $(this).attr("alt");
		        if (confirm("确定删除该分类？")) {
		    ...
		    $("#delBatchBtn").click(function(){
		        ...
		        if (confirm("确定删除所选分类?")) {
		            ...
		            var url = "${appDomain}/admin/xhxw/delete_xhxwArray.htm"
		            $.get(url, {
		                idArray: value
		            }, function(result){
		                alert("操作成功");
		...
		var url = "${appDomain}/admin/xhxw/xhxw_detail.htm?id=" + messageId;
    ...
    <body>
        <div id="main">
            <div id="content">    
             <h3 class="title">协会新闻列表页面</h3>              
                <form id="xhxwForm" name="xhxwForm" action="${appDomain}/admin/xhxw/list.htm" method="GET">
	              <div class="search">
					<ul>
	                ...
	                <div class="list">                    
                     <div class="opt-btn">
	                    ...
	                    <table class="list-info">
	                        ...
	                        <#list page.xhxwList as xhxw>
								<tr>
									<td width="3%">
		                                <input type="checkbox" name="chkItem" value=${xhxw.id} />
		                            </td>
									<td>${xhxw.title}</td>
									<td>${xhxw.sortName}</td>
									<td>${xhxw.cc}</td>
									<td>${xhxw.releaseDate}</td>
		                            <td class="td-btn" width="20%">
		                            	<a class="action-opt act-update" href="#" title="更新" alt="${xhxw.id}">修改</a>
										<a class="action-opt act-delete" href="#" title="删除" alt="${xhxw.id}">删除</a>		                           
									</td>
								</tr>
							</#list>
	                        </tr>
