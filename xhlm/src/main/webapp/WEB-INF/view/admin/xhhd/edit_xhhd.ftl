		...
		$(document).ready( function() {				
			...
			//初始化xhEditor编辑器插件
				$("#ct").xheditor({
					...
					skin:'default',
					upMultiple:false,
					upLinkUrl:"${appDomain}/admin/upload_image.htm?dir=xhhd",
					upLinkExt:"doc,docx,xls,xlsx,txt,rar,zip,pdf",
					upImgUrl: "${appDomain}/admin/upload_image.htm?dir=xhhd",
...
<body>
	<div id="main">
		<div id="content">
			<h3 class="title">修改协会活动</h3>
			<form class="f0" action="${appDomain}/admin/xhhd/edit_xhhd.htm" method="POST">
			<input type="hidden" name="id" value="${xhhd.id}" />
				<table class="tbls">
			  <tr>
			    <th >标题:</th>
			    <td><input class="text" type="text" name="title" maxlength="20" value="${xhhd.title}"/>
				</td>
			  </tr>
			    <tr>
			    <th>行业:</th>
			     ...
			     <select name="sortId" class="sees">			    
                	<#list sortdetailList as sortdetail>
						<option value="${sortdetail.id}" <#if xhhd.sortId == sortdetail.id>selected="selected"</#if> >${sortdetail.name}</option>
					</#list>
                 </select>
				</td>
			  </tr>
			     <tr>
			    <th>周期:</th>
			     <td><input class="text" type="text" name="zq" maxlength="20" value="${xhhd.zq}" />
				</td>
			  </tr>
			  <tr>
			    <th>要求:</th>
			     <td><input class="text" type="text" name="yq" maxlength="20" value="${xhhd.yq}" />
				</td>
			  </tr>
			  <tr>
			    <th>招牌:</th>
			     <td>
					<img src="" id="imgZp" width="100px" height="125px" value="${xhhd.zp}" /><br />
					<input width="60" type="hidden" id="zp" name="zp" value="${xhhd.zp}">
			    ...
			    <th>内容:</th>
			     <td>
			     	<textarea id="ct" name="content" rows="25" style="width:100%; border: 1px">${xhhd.content}</textarea>
				</td>
			  </tr>
			</table>
