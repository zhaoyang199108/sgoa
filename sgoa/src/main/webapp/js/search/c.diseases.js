(function($) {
	...
	$.diseases = function(input, options) {
		...
		function processKey(e) {
			...
			if ((/27$|38$|40$/.test(e.keyCode) && $results.is(':visible'))
				...
				timeout = setTimeout(diseases, options.delay);
					...
					success : function(_data) {
						...
						$.each(
							...
							function(h, entry) {
								html += '<li rel="'
										+ entry['id']
										+ '"><a alt="'+entry['id']+'" style="display:block;width:100%;" href="#'
										+ h + '">'
										+ entry['fileNum']
										+ '</a></li>';
							});
		...
		function selectCurrentResult() {
			...
			if ($currentResult) {
				...
				$.ajax({
		            ...
		            url: '/docin/fileNum_docin.htm',
		            dataType: 'json',
					data: 'fileNum='+$("#fileNum").val()+'&r=' + Math.random(),
					success: function(data){
						// 从Controller里取得对象
		            	var docinObj = data.docin;
		            	$('#fondsNum').val(docinObj.fondsNum);
		            	$('#fileDept').val(docinObj.fileDept);
		            	$('#urgent').val(docinObj.urgent);
		            	$('#receiverTime').val(docinObj.receiverTime);
		            	$('#blqx').val(docinObj.blqx);
		            	$('#supervise').val(docinObj.supervise);
		            	$('#title').val(docinObj.title);
		            	$('#srcFileName').val(docinObj.srcFileName);
		            	$('#fileDir').val(docinObj.fileDir);
		            	$('#fileDirDiv').html(docinObj.fileDir);
		            }
		        	});
			}
		}
