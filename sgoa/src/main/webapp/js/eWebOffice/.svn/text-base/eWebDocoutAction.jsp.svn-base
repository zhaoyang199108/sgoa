...
<%!
	...
	public void DoLoadFile()
    {
    	...
    	EWEBOS.WebMsgClear();
    	if(s_RecordID!=""){
        	String sql = "select * from eWebDocout_T_DocumentFile where D_RecordID=?";
        	try{
    ...
    {	
        ...
        String sql = " select count(*) from eWebDocout_T_DocumentFile where D_RecordID=?";
        EWEBOS.WebMsgClear();
        try{
        	...
        	if(n_count>0){
        		sql = "update eWebDocout_T_DocumentFile set D_RecordID=?,D_FileName=?,d_FileType=?,D_FileSize=?,D_FileBody=?,D_FilePath=?,D_UserName=?,D_Descript=? where D_RecordID='"+s_RecordID+"'";
        	}else{
        		sql=  "insert into eWebDocout_T_DocumentFile(D_RecordID,D_FileName,D_FileType,D_FileSize,D_FileBody,D_FilePath,D_UserName,D_Descript) values(?,?,?,?,?,?,?,?)";
        	}
        	stmt = conn.prepareStatement(sql);
        	stmt.setString(1,s_RecordID);
    ...
    public void DoListVersion()
    {
    	...
    	if(!s_RecordID.equals("")){
			String sql = "select * from eWebDocout_T_VersionFile where D_RecordID=?"; 
			try{
				stmt = conn.prepareStatement(sql);
				stmt.setString(1,s_RecordID);
				rs = stmt.executeQuery();
    ...
    public void DoSaveVersion()
    {
        ...
        if (s_RecordID != "")
        {
            String sql = "insert into eWebDocout_T_VersionFile(D_RecordID,D_FileName,D_Filetype,D_Filesize,D_FileDate,D_FileBody,D_FilePath,D_UserName,D_Descript)values(?,?,?,?,?,?,?,?,?)";
