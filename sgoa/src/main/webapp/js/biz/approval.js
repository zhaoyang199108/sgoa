...
function setNewSaveTrAttribute(newTr, i) {
	...
	newTr.find("input[name=positionIds]").attr("id", 'positionId' + i); // 审批人
	newTr.find("select[name=statuss]").attr("id", 'positionId' + i); // 是否有权限
}
...
function clearSaveTrValue(tr) {
	...
	tr.find("input[name=positionIds]").val(""); // 审批人
