...
// *************************************追加结束*********************************
	...
	/**
	 * 提交按钮按下-2011-09-05-追加开始
	 */
	$("#subBtn").click(function() {

		var hasErrors = false;
		$("select[name=goodsNames]").each(function() {
...
function setNewTrAttribute(newTr, i) {
	...
	newTr.find("input[name=specifications]").attr("id", 'specification' + i); // 规格输入框
	...
	newTr.find("input[name=lossResults]").attr("id", 'lossResult' + i); // 报损原因输入框
	newTr.find("input[name=remarks]").attr("id", 'remark' + i); // 备注输入框
	// 设置默认焦点为条形码
	newTr.find("input[name=goodsNames]").attr("id", 'goodsName' + i);
}
...
function clearGoodsTrValue(tr) {
	...
	tr.find("input[name=specifications]").val(""); // 规格输入框
	...
	tr.find("input[name=lossResults]").val(""); // 报损原因输入框
