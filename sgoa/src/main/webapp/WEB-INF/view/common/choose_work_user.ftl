		...
		<script type="text/javascript">
			...
			function zTreeOnClick(event, treeId, treeNode){
	            ...
	            $.ajax({
	                ...
	                url: '${appDomain}/common/get_dept_work_user.htm',
	                dataType: 'json',
		    ...
		    $(document).ready(function(){
		        ...
		        $.ajax({
		            type: 'GET',
		            contentType: 'application/json',
		            url: '${appDomain}/common/find_dept.htm',
		            dataType: 'json',
		            data: 'r=' + Math.random(),
		            success: function(data){
		            	// 从Controller里取得对象数组
		        ...
		        $("#oppBtn").click(function(){
		            ...
		            var checkedSize = $("input[name=userId]:checked").length; // 选中数量
		            var allSize = $("input[name=userId]").length; // 全部数量
		     ...
		     * 点击部门下的用户时,根据操作将该用户添加至已选择列表或者删除
		     */
		    function selectOneUser(userId, obj){
		        // 判断操作事件,选中还是取消
		        if ($(obj).attr("checked")) {
		            appendSelectUser(userId, obj);
		        }
		        else {
		    ...
		    function appendDeptUserList(data){
		        ...
		        $.each(data, function(i, user){
		            ...
		            var li = "<li><input type=\"checkbox\" name=\"userId\" value=\"" + user.id + "\" ";
		            if (isExistInList(user.id)) {
		                li = li + "checked=\"checked\" ";
		            }
		            li = li + "onclick=\"selectUser('" + user.id + "', this)\" />";
		    ...
		    function isExistInList(id){
		        ...
		        $(".select-list li:not(:first)").each(function(){
		            var userId = $(this).find("a.opt").attr("alt");
		            if (id == userId) {
		                isExist = true;
		                return;
		            }
		    ...
		    function removeUser(userId){
		        ...
		        $(".user-list li:not(:first)").each(function(){
		            var checkbox = $(this).find("input[name=userId]");
		            if (userId == checkbox.val()) {
		    ...
		    function setAllAndOppBtn(){
		        var checkedSize = $("input[name=userId]:checked").length; // 选中数量说
		        var allSize = $("input[name=userId]").length; // 全部数量
    		...
    		<div class="wrap">
		        ...
		        <ul class="select-list">
		            ...
		            <#list selUserList as selUser>
			            <li id="li_${selUser.id}">
			                <span class="uname">${selUser.userName}</span>
			                <span class="udept">${selUser.deptName}</span>
			                <a class="opt" href="#" alt="${selUser.id}" onclick="removeSelectUser('${selUser.id}')">删除</a>
			            </li>
		            </#list>
		        </ul>
	     	</div>
