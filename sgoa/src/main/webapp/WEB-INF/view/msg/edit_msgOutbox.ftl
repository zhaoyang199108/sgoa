...
<body>
	...
	<div class="main_nav">
	 <#include "msg/menu.ftl">
		<div id="content">
		<h3 class="title"><font size="3"><strong>站内信更新</strong></font></h3>
			<form class="f0" action="${appDomain}/msg/edit_msgOutbox.htm" enctype="multipart/form-data" method="POST">
			   <input class="txt title" type="hidden" id="id" name="id" value="${msgOutbox.id}" />
			   <input class="txt title" type="hidden" id="textId" name="textId" value="${msgOutbox.textId}" />
				<div class="ct">
				<table class="ftb">
					<tr>					    
						 <th>收件人:</th>
						 <td>
		                 <input class="txt title" type="hidden" id="receiverIds" name="receiverIds" value="${msgOutbox.receiverIds}" />
					...
					<tr>					 
						<th>标题:</th>
						<td>
						<input class="txt title" type="text" id="title" name="title" value="${msgOutbox.title}" maxlength="50"/>
						</td>
					</tr>
					...
					<th>内容:</th>
						<td>
							<textarea id="ct"  class="woe" style="width:95%;height:200px;" name="msgText" maxlength="500">${msgOutbox.msgText}</textarea>
						</td>
					</tr>
					<#if loginId=="ybgs">
					<tr>					 
