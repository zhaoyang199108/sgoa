		...
		$(document).ready( function() {
			...
			$("#saveBtn").click( function() {
			   var title=$('#title').val();
				...
				$('form').attr("action", "${appDomain}/msg/write_add.htm");	
				$('form').submit(); 	   
			});
...
<body>
	...
	<div class="main_nav">
	 <#include "msg/menu.ftl">
		<div id="content">
		<h3 class="title"><font size="3"><strong>站内信回复</strong></font></h3>
			<form class="f0" action="${appDomain}/msg/replay_msgInbox.htm" enctype="multipart/form-data" method="POST">   
				<div class="ct">
				<table class="ftb">
					<tr>					    
						 <th>收件人:</th>
						 <td>
		                 <input class="txt title" type="hidden" id="receiverIds" name="receiverIds" value="${msgInbox.senderId}" />
					...
					<tr>					 
						<th>标题:</th>
						<td>
						<input class="txt title" type="text" id="title" name="title" value="回复:${msgInbox.title}" maxlength="50"/>
						</td>
					</tr>
					...
					<th>内容:</th>
						<td>
							<textarea id="ct"  class="woe" style="width:95%;height:200px;" name="msgText" maxlength="500">${msgInbox.msgText}</textarea>
						</td>
					</tr>
					<#if loginId=="ybgs">
					<tr>					 
