	        ...
	        $.ajax({
	            ...
	            success: function(data){
	            	...
	            	for (var i=0;i<deptListObj.length;i++) {
	            		...
	            		if (dept.id == $("#deptId").val()) {
	            			zNodes[i] = {id:dept.id,pId:dept.unitId,name:dept.deptName, open:true,checked:true};
	            		} else {
...
<body>
	<div class="sow">
		<h3 class="title"><font size="3"><strong>用户权限</strong></font></h3>
		<form class="f0" name="roleForm" action="${appDomain}/user/add_user_role.htm" method="POST">
