...
<body>
     <div class="sow">
			...
			<form class="f0" action="${appDomain}/scheduler/edit_scheduler_one.htm" method="POST">
				<input type="hidden" name="id"  value="${scheduler.id}"/>
				<table class="ftb">
				<tr>
				    <th>日程开始时间:</th>
					<td>						
						<input class="text" type="text" name="startTime" value="${scheduler.startTime}" readonly="readonly"/>
					</td>
					<th>日程结束时间:</th>
					<td>						
						<input class="text" type="text" name="endTime" value="${scheduler.endTime}" readonly="readonly"/>
					</td>
				</tr>
				<tr>
				    <th>开始提醒时间:</th>
					<td>						
						<input class="text" type="text" id="startRemindTime" name="startRemindTime" value="${scheduler.startRemindTime}" />
					</td>
					<th>是否继续提醒:</th>
					<td>						
						<select id="isRemind" name="isRemind">
                					<option value='Y' <#if scheduler.isRemind == "Y">selected="selected"</#if>>提醒</option>
                         </select>
					</td>
				</tr>				
				<tr>
				    <th>日程内容:</th>
					<td colspan="3">						
						<textarea name="content" id="ct"  class="woe"  readonly="readonly" >${scheduler.content}</textarea>
					</td>
				</tr>
