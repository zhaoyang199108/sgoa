			...
			$("input[name=receiverTime]").click(function(){
			 ...
			 /**
                ...
                $("#draftBtn").click(function(){
                	...
                	// 添加校验
                	$('form').attr("action", "${appDomain}/sw/draft_sw.htm?edit=true");
                    $('form').submit();
                });
		...
		<div id="main">
          <div class="main_nav">
            ...
            <div class="pstty"><h4>文件处理单</h4>
		        <form class="f0" action="${appDomain}/sw/edit_sw.htm" enctype="multipart/form-data" method="POST">
		        	<input type="hidden" id="currentOptId" name="currentOptId" value="${sw.currentOptId}" />
		        	...
		        	<input type="hidden" id="id" name="id" value="${sw.id}" />
		        	<input type="hidden" id="srcFileName" name="srcFileName" value="${sw.srcFileName}" />
		        	<input type="hidden" id="fileDir" name="fileDir" value="${sw.fileDir}" />
		        	<table class="stty">
		        	<tr class="yty">
					    <td colspan="3" class="rre">全宗号：					     
					      <input type="text" name="fondsNum" value="${sw.fondsNum}"/> </td>
					   <td colspan="3" class="rre">
					       全宗名称：					      
					      <input type="text" name="fondsName" value="${sw.fondsName}"/>
					      </td>
					    </tr>
					  <tr>
					    <th width="9%">来文单位：</th>
					    <td>
					     <input type="text" name="fileDept" value="${sw.fileDept}"/>
					     </td>
					     <th width="9%">文件编号：</th>
					    <td>
					     <input type="text" name="fileNum" value="${sw.fileNum}"/>
					     </td>
					     <th width="9%">缓急程度：</th>
					    <td>
					       <select name="urgent">
        					<option value=1 <#if sw.urgent == 1>selected="selected"</#if>>正常</option>
                          	<option value=2 <#if sw.urgent == 2>selected="selected"</#if>>加急</option>
                          	<option value=3 <#if sw.urgent == 3>selected="selected"</#if>>特急</option>
                          	<option value=4 <#if sw.urgent == 4>selected="selected"</#if>>紧急</option>
                          	<option value=5 <#if sw.urgent == 5>selected="selected"</#if>>特提</option>
                         </select>
                         </select>
					     </td>
					  </tr>
					  <tr>
					    <!--<th width="9%">成文时间：</th>
					    <td>
					     <input type="text" name="textTime" id="textTime" value="${sw.textTime}"/>
					     </td>-->
					     <th width="9%">收文时间：</th>
					    <td>
					     <input type="text" name="receiverTime" id="receiverTime" value="${sw.receiverTime}"/>
					     </td>
					     <th width="9%">登记号：</th>
					    <td>
					     <input type="text" name="registerNum" value="${sw.registerNum}"/>
					     </td>
					     <th width="9%">密　　级：</th>
					    <td>
					       <select name="security" id="security">
        					<option value=1 <#if sw.security == 1>selected="selected"</#if>>普通</option>
                          	<option value=2 <#if sw.security == 2>selected="selected"</#if>>加密</option>
                          </select>
					     </td>
					    </tr>
					  <tr>
		                    <th>督　　办：</th>
					    	<td colspan="5"><input type="checkbox" name="supervise" <#if sw.supervise==1> checked=true</#if> value="1" style="width:auto;"/>是否列为督办</td>
		                </tr>
					  <tr>
					    <th>文件标题：</th>
					    <td colspan="5"><input type="text" name="title" class="wid" value="${sw.title}"/></td>
					    </tr>
					  <tr>
					    <th>收件人：</th>
					    <td colspan="5">
					     <input type="hidden" id="receiverId" name="receiverId" value="${sw.receiverId}" />
		                 <input id="receiverIds" name="receiverIds" type="text" value="${sw.receiverName}" readonly="readonly" />
					  ...
					  <tr>
					    ...
					    <tr>
						<td colspan="5">
	                    	<div id="fileDirDiv" style="height:30px;border: 2px solid green;overflow-y:scroll;overflow-x:hidden; width:95%;">${sw.srcFileName}</div>
	                    </td>
					  </tr>	
		                <tr class="act">
		                    <td colspan="6">
