...
<script type="text/javascript" for="eWebOffice1" event="OnLoad()">
	...
	eWebOffice1.RecordID = "${msgoneOutbox.dRecordid}";
...
<body>
	...
	<div class="main_nav">
	 <#include "msgone/menu.ftl">
		<div id="content">
		<h3 class="title"><font size="3"><strong>文稿拟订更新</strong></font></h3>
			<form class="f0" action="${appDomain}/msgone/edit_msgoneOutbox.htm" enctype="multipart/form-data" method="POST">
			   <input class="txt title" type="hidden" id="id" name="id" value="${msgoneOutbox.id}" />
			   <input class="txt title" type="hidden" id="textId" name="textId" value="${msgoneOutbox.textId}" />
			   <input type="hidden" id="dRecordid" name="dRecordid" value="${msgoneOutbox.dRecordid}" />
				<div class="ct">
				<table class="ftb">
					<tr>					    
						 <th>收件人:</th>
						 <td>
		                 <input class="txt title" type="hidden" id="receiverIds" name="receiverIds" value="${msgoneOutbox.receiverIds}" />
					...
					<tr>					 
						<th>标题:</th>
						<td>
						<input class="txt title" type="text" id="title" name="title" value="${msgoneOutbox.title}" maxlength="50"/>
						</td>
					</tr>
