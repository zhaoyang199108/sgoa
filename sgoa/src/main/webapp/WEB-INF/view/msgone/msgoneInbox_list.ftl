...
<html>
    <head>
		...
		$(document).ready(function(){
			 $("#userNamePy").suggest({attachObject:'#suggest',url:'${appDomain}/megoneUser/user_list.htm'});
			...
			$("input[name=createDate]").click(function(){
		      ...
		      // 转发按钮点击事件
		    $(".edit").click(function(){
		        var url = "${appDomain}/msgoneInbox/edit_msgoneInbox.htm?id=" + $(this).attr("alt") + "&r=" + Math.random();
		    ...
		    // 删除按钮点击事件
		    $(".del").click(function(){
		        var url = "${appDomain}/msgoneInbox/delete.htm?id=" + $(this).attr("alt");
		        if (confirm("确定删除该信息？")) {
		    ...
		    $(".delete-btn").click(function(){
		        ...
		        if (item.length == 0) {
		          ...
		          if (confirm("确定删除所选信息？")) {
		            ...
		            var url = "${appDomain}/msgone/delete_msgoneInboxArray.htm"
		            $.get(url, {
		                idArray: value
		            }, function(result){
		                alert("操作成功");
		   ...
		   // 已读按钮点击事件
		    $(".sign").click(function(){
		        var url = "${appDomain}/msgoneInbox/read.htm?id=" + $(this).attr("alt");
		        if (confirm("确定读取该信息？")) {
		            $.get(url, function(result){
		                alert("操作成功");
		                $('form').submit();
		    ...
		    $(".sign-btn").click(function(){
		        ...
		        if (item.length == 0) {
		          ...
		          if (confirm("确定读取所选信息？")) {
		            ...
		            var url = "${appDomain}/msgone/read_msgoneInboxArray.htm"
		            $.get(url, {
		                idArray: value
		            }, function(result){
		                alert("操作成功");
    ...
    <body>
        ...
        <div class="main_nav">
         <#include "msgone/menu.ftl">
            <div id="content">                     
                <form id="listForm" name="listForm" action="${appDomain}/msgone/msgoneInbox_list.htm" method="GET">
                   <div class="ct-top">
	                   ...
	                   <div class="ct">
	                    <table class="list">
	                        ...
	                        <#list page.msgoneInboxList as msgoneInbox>
								<tr>
									<td>
		                                <input type="checkbox" name="chkItem" value=${msgoneInbox.id} />
		                            </td>
									<td class="left">
										<#if msgoneInbox.isRead == "Y">
											<label style="width:280px;"><a href="${appDomain}/msgone/detail_msgoneInbox.htm?id=${msgoneInbox.id}">${msgoneInbox.title}</a></label>
										<#else>
											<strong><label style="width:380px;">${msgoneInbox.title}</label></strong>
										</#if>
									</td>
									<td class="left">${msgoneInbox.senderName}</td>
									<td>${msgoneInbox.createDate?datetime}</td>
									<td><#if msgoneInbox.isRead == "Y">
											${msgoneInbox.updateDate?datetime}
										<#else>
											<strong>-</strong>
										</#if></td>	
								    <td class="td-btn" width="10%">
                                  	  	<#if msgoneInbox.isRead == "Y">
											<a class="act-btn"  title="读取"  href="#" alt="${msgoneInbox.id}">读取</a>
										<#else>
											<a class="act-btn sign"  title="读取"  href="#" alt="${msgoneInbox.id}">读取</a>
