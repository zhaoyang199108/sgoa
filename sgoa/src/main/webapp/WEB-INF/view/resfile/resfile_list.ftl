		...
		$(document).ready(function(){
		   // 日期控件
		    ...
		    $(".update").click(function(){
		        var url = "${appDomain}/resFile/edit.htm?id=" + $(this).attr("alt") +"&r=" + Math.random();
		        var param = 'dialogWidth=760px;dialogHeight=420px;status=no;center=yes;scroll=no';
		        var val = window.showModalDialog(url, '', param);
		           if (val == 'refresh') {
		            alert("操作成功");
		    ...
		    $(".act-noShare").click(function(){
		        var url = "${appDomain}/resFile/noShare.htm?id=" + $(this).attr("alt") +"&r=" + Math.random();
		        if (confirm("确定取消共享？")) {
			...
			$("table.list tr:not(:first)").dblclick(function(){
			   ...
			   var url = "${appDomain}/resFile/dbdetail.htm?id=" + $(this).find("input[name=chkItem]").val() + "&r=" + Math.random();
		     ...
		     // 删除按钮点击事件
		    $(".del").click(function(){
		        var url = "${appDomain}/resFile/delete.htm?id=" + $(this).attr("alt") +"&r=" + Math.random();
		        if (confirm("确定删除该公告？")) {
		            $.get(url, function(result){
		            if(result){
		                alert("操作成功");
		    ...
		    $("#delBatchBtn").click(function(){
		        ...
		        if (confirm("确定删除所选公告？")) {
		            ...
		            var url = "${appDomain}/resFile/delete_batch.htm"+ "?r=" + Math.random();
		            $.get(url, {
		                idArray: value
		            }, function(result){
		                if(result){
    ...
    <body>
        ...
        <#include "resfile/menu.ftl">
            <div id="content">
                ...
                <form id="listForm" name="listForm" action="${appDomain}/resFile/list.htm" method="GET">
					 <div class="btn-bk">
	                        ...
	                        <#list page.resFileList as resFile >
								<tr>
									...
									<td class="td-btn" width="15%"> 
										<a class="act-btn act-noShare" href="#" alt="${resFile.id}">取消</a>									
			                        	<a class="act-btn share" title="共享" href="${appDomain}/resFile/detail.htm?id=${resFile.id}" target="mainFrame">共享</a>
										<#if resFile.isShore=='Y'><a class="act-btn" href="#" alt="${resFile.id}">更新</a>
										<#else><a class="act-btn update" href="#" alt="${resFile.id}">更新</a></#if>
									</td>
								</tr>
							</#list>
	                        </tr>
