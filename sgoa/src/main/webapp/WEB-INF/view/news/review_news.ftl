	...
	<script type="text/javascript">
            $(document).ready(function(){
                ...
                $("#draftBtn").click(function(){
                	...
                	// 添加校验
                	$('form').attr("action", "${appDomain}/news/add_news_review.htm");
                	 if(DoCheckSubmit())
                    {
                    if(SaveSignature()) {
                    	$('form').submit();
		    ...
		    $("#rebackBtn").click(function(){
                    ...
                    // 添加校验
                    // 选择提交返回人
                    var url = "${appDomain}/common/choose_reback_user.htm?step="+$("#step").val()+"&id=" + $(this).attr("alt") + "&r=" + Math.random();
                    var param = 'dialogWidth=380px;dialogHeight=380px;status=no;center=yes;scroll=no';
                    ...
                    $('form').attr("action", "${appDomain}/news/news_reback.htm");
		    		 if(DoCheckSubmit())
                    {
                    	if(SaveSignature()) {
                    	$('form').submit();
...
<div id="main">
	...
	<#include "news/menu.ftl">
		<div id="content">
			 <form class="f0" action="${appDomain}/news/add_news_review.htm" method="POST">
			  <div class="ct">
				<table class="ftb">
            		...
            		<input type="hidden" id="d_templateid" name="d_templateid" value="${news.dRecordid}" />
				  ...
				  <tr>
		                    ...
		                    <td colspan="3">
		                        <input class="title" type="text" id="title" name="title" value="${news.title}" readonly="readonly"/>
		                    </td>
		                </tr>
		                 <tr>
		                    <th> 主题词:</th>
		                ...
		                <tr>
		                    ...
		                    <td>
		                        <input class="txt" type="text" value="${news.createDate?string("yyyy-MM-dd HH:mm")}" readonly="readonly"/>
		                    </td>
		                </tr>
		               <tr>
		                   <th>
		                    ...
		                    <td>
		                        <input class="txt" type="text" id="textTime" name="textTime" value="" readonly="readonly"/>
		                    </td>
		                </tr>
		                <tr>
				     	<th>备注:</th>
				...
				<#list onePage.newsReviewList as newsReview>
				  <tr>
				    <th>审核意见：
				    ...
				    <td colspan="3">
				    <label>
					<OBJECT id="SendOutView${newsReview_index}" classid="clsid:2294689C-9EDF-40BC-86AE-0438112CA439" codebase="${jsDomain}/js/iWebRevision/iWebRevision.cab#version=6,6,0,294" width=100% height=100%>
			    ...
			    <tr>
					...
					<label>
					 <OBJECT id="SendOut" name="SendOut" classid="clsid:2294689C-9EDF-40BC-86AE-0438112CA439" codebase="${jsDomain}/js/iWebRevision/iWebRevision.cab#version=6,6,0,294" width=100% height=100%>
                      ...
                      <param name="ShowPage" value="2">    <!-- ShowPage:设置默认显示页面，0:电子印章,1:网页签批,2:文字批注  默认值:0  -->
                      <param name="InputText" value="">    <!-- InputText:设置署名信息，  为空字符串则默认信息[用户名+时间]内容  -->
                      <param name="PenWidth" value="1">     <!-- PenWidth:笔的宽度，值:1 2 3 4 5   默认值:2  -->
                      <param name="FontSize" value="12">    <!-- FontSize:文字大小，默认值:11 -->
                      <param name="SignatureType" value="2">    <!-- SignatureType:签章来源类型，0表示从服务器数据库中读取签章，1表示从硬件密钥盘中读取签章，2表示从本地读取签章，并与ImageName(本地签章路径)属性相结合使用  默认值:0 -->
                      <param name="InputList" value="同意\r\n不同意\r\n请上级批示\r\n请速办理">    <!-- InputList:设置文字批注信息列表  -->
                    </OBJECT>
					</label>
					<br /><br />
				  ...
				  <tr class="act">
                    ...
                    <#if approvalDetailForStatus == 'Y'>
		             ...
		             <#if news.draftsId!=news.nextOptId>
		             <input id="rebackBtn" class="btn" name="draftBtn" type="button" alt="${news.id}" value="返回修改" />
		              </#if>
                     <input id="clearBtn" class="btn" name="clearBtn" type="button" value="同意修订" />
                    </td>
                </tr>
		</table>
	</div>
