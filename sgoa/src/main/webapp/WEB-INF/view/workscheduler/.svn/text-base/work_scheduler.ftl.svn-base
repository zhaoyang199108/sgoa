	    ...
	    <script type="text/javascript">
			...
			var addEvent = function addEvent(event_id, e)
		    {
		    	if(e.text == '' || e.text == null){
		    		alert('标题内容不能为空');
		    		// 设置日程,删除日程成功
		            window.location.href = "${appDomain}/workscheduler/work_scheduler.htm";
		    		return;
		    	}
		    	programing = true;
		        $.ajax({
		            ...
		            url: '/workscheduler/add_workscheduler.htm',
		            dataType: 'json',
		     ...
		     var changeEvent = function changeEvent(event_id, e){
		    	...
		    	programing = true;
		     	$.ajax({
		            type: 'POST',
		            contentType: 'application/json',
		            url: '/workscheduler/edit_workscheduler.htm',
		            dataType: 'json',
		            contentType: "application/x-www-form-urlencoded; charset=utf-8",
		            data: 'id=' + event_id + '&content=' + encodeURI(e.text) + '&startTime=' + e.start_date.pattern("yyyy-MM-dd HH:mm:ss") + '&endTime=' + e.end_date.pattern("yyyy-MM-dd HH:mm:ss") + '&r=' + Math.random(),
		            success: function(data){
		     ...
		     deleteEvent = function(event_id, e){
		     	$.ajax({
		            type: 'POST',
		            contentType: 'application/json',
		            url: '/workscheduler/delete_workscheduler.htm',
		            dataType: 'json',
		            contentType: "application/x-www-form-urlencoded; charset=utf-8",
				...
				if (scheduler.isCalendarVisible())
					...
					scheduler.renderCalendar({
						...
						handler:function(date,calendar){
							...
							scheduler.destroyCalendar()
							var url = "${appDomain}/workscheduler/work_scheduler.htm?selectTime=" + date.pattern("yyyy-MM-dd HH:mm:ss");
