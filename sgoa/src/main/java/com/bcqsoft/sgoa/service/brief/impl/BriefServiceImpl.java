package com.bcqsoft.sgoa.service.brief.impl;
 ...
 */
@Service
public class BriefServiceImpl extends BaseService implements BriefService {
	...
	public boolean addBriefDraftInfo(News news) {
		// 设置信息状态为草稿状态
		news.setEnabled(CommonChar.FILE_DRAFT);
		news.setSort(CommonChar.SORT_JB);
	 ...
	 */
	@Transactional(readOnly = false, rollbackFor = DataAccessException.class)
	public boolean updateBriefDraftInfo(News news) {
	 ...
	 */
	@Transactional(readOnly = false, rollbackFor = DataAccessException.class)
	public boolean addBriefInfo(News news) {
		// 设置信息状态为正常状态
		...
		if (news.getNextOptId() != null && !"".equals(news.getNextOptId())) {
			...
			remind.setUrl("brief/edit_brief_review.htm?id=" + pk);
	 ...
	 * @Date 2011-12-29
	 */
	public NewsPage getMyDraftBriefList(NewsPage newsPage) {
		// 根据条件查找用户信息拟稿数量
	 ...
	 * @Date 2011-12-29
	 */
	public NewsPage getApprovedBriefList(NewsPage newsPage) {
		// 根据条件查找用户待审核信息数量
	 ...
	 */
	@Transactional(readOnly = false, rollbackFor = DataAccessException.class)
	public boolean removeOneBriefInfo(Long id) {
	 ...
	 */
	@Transactional(readOnly = false, rollbackFor = DataAccessException.class)
	public boolean removeOneBriefInfo(long[] longArray) {
		boolean returnValue = false;
		// 循环删除收件箱
		// 如记录数量较多时,可以采用Ibatis中的Batch处理
		for (long storageId : longArray) {
			// 逻辑删除,更新状态为不可用
			boolean isUpdate = removeOneBriefInfo(storageId);
			// 某条更新成功即设置操作成功
			if (isUpdate) {
				returnValue = true;
			}
	 ...
	 * @Date 2012-1-6
	 */
	public News getUserDraftBriefDetail(Long id) {
	 ...
	 */
	@Transactional(readOnly = false, rollbackFor = DataAccessException.class)
	public boolean updateBriefInfo(News news) {
		// 更新信息信息
		Integer count = newsDAO.updateNewsInfoById(news);
		// 添加这条数据的历史记录
	 ...
	 */
	@Transactional(readOnly = false, rollbackFor = DataAccessException.class)
	public boolean updateOneBriefInfo(Long id) {
	 ...
	 */
	@Override
	public NewsReviewPage getBriefReviewListById(Long id) {
	 ...
	 */
	@Override
	public NewsReviewPage getBriefReviewListByIdOne(Long id) {
		NewsReviewPage page = new NewsReviewPage();
	 ...
	 */
	@Override
	public NewsPage getBriefReviewList(NewsPage newsPage) {
		// 根据条件查找用户待审核信息数量
	 ...
	 */
	@Override
	public NewsPage getMyBriefReviewList(NewsPage newsPage) {
		// 根据条件查找用户待审核信息数量
	 ...
	 */
	@Transactional(readOnly = false, rollbackFor = DataAccessException.class)
	public boolean createBriefReview(News news) {
		...
		if (news.getNextOptId() != null && !"".equals(news.getNextOptId())) {
			...
			remind.setTitle(news.getTitle());
			remind.setStatus(CommonChar.REMIND_STATUS_TX);
			remind.setUrl("brief/edit_brief_review.htm?id=" + news.getId());
			remindDAO.insertIntoRemind(remind);
		}
		// 此处默认返回操作成功成功
		return isInsertSucc(pk);
	 ...
	 */
	@Override
	public List<NewsStep> getBriefStepList(Long id) {
	 ...
	 */
	@Transactional(readOnly = false, rollbackFor = DataAccessException.class)
	public boolean createBriefReback(News news) {
		// 数据库新增一条审核记录,并返回是否插入成功
		news.setCurrentOptId(SecurityUtils.getLoginId());
		news.setEnabled(1);
	 ...
	 * @Date 2011-12-22
	 */
	public NewsPage getBriefSearchList(NewsPage outPage) {
