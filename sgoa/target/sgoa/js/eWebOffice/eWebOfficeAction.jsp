<%@ page contentType="text/html; charset=UTF-8" language="java" import="java.util.*,com.ewebsoft.eweboffice.*,java.sql.*,java.util.Date,java.text.SimpleDateFormat" pageEncoding="UTF-8"%>
<%!
    ...
    {	
        ...
        try{
        	...
        	if(n_count>0){
        		sql = "update eWebOffice_T_DocumentFile set D_RecordID=?,D_FileName=?,d_FileType=?,D_FileSize=?,D_FileBody=?,D_FilePath=?,D_UserName=?,D_Descript=? where D_RecordID='"+s_RecordID+"'";
        	}else{
        		sql=  "insert into eWebOffice_T_DocumentFile(D_RecordID,D_FileName,D_FileType,D_FileSize,D_FileBody,D_FilePath,D_UserName,D_Descript) values(?,?,?,?,?,?,?,?)";
        	}
        	stmt = conn.prepareStatement(sql);
        	stmt.setString(1,s_RecordID);
    	...
    	if(!s_RecordID.equals("")){
			...
			try{
				...
				while(rs.next()){
                    ...
                    EWEBOS.WebMsgSetString("Descript" + i, rs.getString("D_Descript"));
    ...
    {
    	int n_FileId = EWEBOS.WebMsgGetString("ID") == "" ? 0 : Integer.parseInt(EWEBOS.WebMsgGetString("ID"));
    	EWEBOS.WebMsgClear();
    	if (n_FileId > 0)
        {
    ...
    public void DoSaveVersion()
    {
        ...
        if (s_RecordID != "")
        {
            String sql = "insert into eWebOffice_T_VersionFile(D_RecordID,D_FileName,D_Filetype,D_Filesize,D_FileDate,D_FileBody,D_FilePath,D_UserName,D_Descript)values(?,?,?,?,?,?,?,?,?)";
    ...
    {	System.out.println(111);
        ...
        if (s_TemplateID != null && !"null".equals(s_TemplateID))
        {
            try{
            	...
            	stmt = conn.prepareStatement(sql);
				stmt.setString(1,s_TemplateID); 
				stmt.setString(2,java.net.URLDecoder.decode(s_FileName,"UTF-8"));
				stmt.setString(3,s_FileType);
				stmt.setInt(4,n_FileSize);
				stmt.setString(5,GetDateTime());
				stmt.setBytes(6,a_FileBody);
				stmt.setString(7,s_FilePath);
				stmt.setString(8,s_UserName);
				stmt.setString(9,java.net.URLDecoder.decode(s_Descript,"UTF-8"));
	            if (stmt.executeUpdate() > 0)
	            {
